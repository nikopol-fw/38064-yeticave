...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if (count($errors)) {
    ...
    $uniq_name = uniqid() . "." . $extension;
    $picture_url = "/img/uploads/" . $uniq_name;
    ...
    if (move_uploaded_file($tmp_name, $upload_path . $uniq_name)) {
      ...
      if (!$db_conf) {
        ...
        mysqli_stmt_bind_param($stmt, 'ssssiii', htmlspecialchars($lot['title']), htmlspecialchars($lot['description']), $picture_url, htmlspecialchars($lot['end_date']), htmlspecialchars($lot['start_price']), htmlspecialchars($lot['bet_step']), htmlspecialchars($lot['category']));
